...
slug: "wedding-it-08-coliseum"
description: "「ケーキ入刀でどちらが先に食べさせる？」に全員がチップを乗せる。競馬と同じ変動オッズ方式、シード金によるオッズ安定化、締切後のドキドキ演出──新郎新婦2人で運営できる大予想システムの全てを解説。"
tags: [NextJS, Supabase, PostgreSQL, GameDesign, Betting]
category: "dev-tips"
author: "tinou"
...
> - ✅ 競馬と同じ「パリミュチュエル方式」のオッズ計算ロジックがわかる
> - ✅ シード金（odds_seed）で初期オッズを安定させるテクニックがわかる
> - ✅ prepared→open→locked→finishedの状態遷移がなぜ重要かわかる
> - ✅ 新郎新婦2人だけで運営できるワンクリック管理画面の設計がわかる
> - ✅ 40人同時ベットでもデータが壊れないDB関数の設計がわかる
...
### 教訓1：数学的な正しさより体験を優先する

最低オッズ保証は、数学的には「配当がプールを超える」という矛盾を生みます。でもゲストの体験としては「勝ったのにガッカリ」を防ぐことのほうが重要。**結婚式のチップは実在しないお金**。体験の質を数学に犠牲にする必要はありません。

### 教訓2：「間」の演出はシステムが作る

`locked` 状態は技術的には不要です。`open` から直接 `finished` にしても動きます。でも「もうベットできない、結果はまだわからない」という**数十秒の沈黙**が、結果発表の興奮を倍増させる。システムの状態遷移で**演出の間**を生み出す発想は、イベント系システムに広く応用できます。

### 教訓3：UIの簡素さ = 参加率

プリセットボタン、ワンタップベット、自動モーダル表示──「考えなくていい」UIほど参加率が上がります。酔っ払ったゲストでも使えることがイベント系アプリの最低ライン。
