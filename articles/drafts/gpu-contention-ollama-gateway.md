...
`ollama ps` を見ると、モデルのkeep-aliveが「4分後に解放」と表示されたまま、**何時間経っても4分後のまま更新され続けている**。つまり誰かが継続的にリクエストを投げている。しかしOllamaのログにはクライアントのプロセス名は残りません。
...
行き詰まって気づいたのは、**全クライアントがすでに同じ場所を通っている**という事実です。Ollamaのポート11434。ここを通らないLLM呼び出しは存在しません。

つまり、Ollama本体を別ポート（11435）に退避させ、**11434にゲートウェイを常駐させれば、既存・将来の全クライアントを無改修で調停できる**のです。
