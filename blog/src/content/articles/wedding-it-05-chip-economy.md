...
description: "結婚式二次会で「誰も損しないゲーム」を作るには？ゲーム理論をベースにしたチップ経済圏の設計と、PayPay風QR送金UI、PostgreSQLトランザクションによるアトミックなチップ移動の実装を解説。"
pubDate: "2026-03-20"
updatedDate: "2026-03-20"
category: "dev-tips"
tags: ["GameDesign", "Supabase", "PostgreSQL", "Economics", "SystemDesign"]
author: "tinou"
---
...
チップ経済圏を設計して一番感じたのは、**ルールは人の行動を「制限」するためではなく「誘導」するためにある**ということです。

「チップが減ったら困る」→ 対戦に参加しなくなる。これは制約が生む悪い行動。
「写真を撮ったらチップがもらえる」→ みんなで写真を撮る。これは報酬が生む良い行動。

同じ問題（チップ不足）に対して、「負けを防ぐ」のではなく「回復手段を増やす」というアプローチを取ったことで、全体的にポジティブな場が作れました。

### DB設計は「正しさ」より「追跡可能性」

`transactions` テーブル経由の設計は、最初は「面倒だな」と感じました。直接UPDATEすれば1クエリで済むのに、なぜわざわざINSERT→トリガーという2ステップを踏むのか。

でも本番で異常が起きたとき、`SELECT * FROM transactions WHERE user_id = ? ORDER BY created_at` 一発で全履歴が見える安心感は絶大でした。「なぜこの人のチップが減ったか」が即座にわかる。**本番でのデバッグ容易性は、開発時の手間を遥かに上回る価値**があります。
